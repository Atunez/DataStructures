...
public class Queue {
	...
	**/
	public QNode dequeue() {
		...
		QNode answer = queue[front];
		front = (front - 1 + size) % size;
		numElements--;
		return answer;	// remove this line once the funciton is completed.
	}
	...
	**/
	public void print() {
		...
		for(int i = 0; i < numElements; i++){
			int ce = (front - i + size) % size;
			if(queue[ce] != null){
				System.out.println(queue[ce]);
			}
		}
	}
	...
	**/
	private void copyArray(QNode [] array) {
		...
		for(int i = 0; i < numElements; i++){
			int nF = (length - i - 1);
			int oF = (front - i + length) % length;
			queue[nF] = new QNode(array[oF]);
		}
